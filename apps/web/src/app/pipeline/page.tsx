  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((l) => {
      if (qq) {
        ...
        const match = [l.nome, l.nomeCorreto, l.telefone, l.whatsapp, l.origem, l.status, l.perfilImovel, l.stageName, ind, String(l.rendaBrutaFamiliar ?? ""), num]
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {view === "LISTA" && (
          <>
            ...
            {activeFilterCount > 0 && (
              <button onClick={clearFilters} className="text-xs text-[var(--shell-subtext)] hover:text-[var(--shell-text)] underline">
                Limpar filtros
              </button>
            )}
            <div className="ml-auto flex items-center gap-2">
              <button onClick={exportCSV} className="rounded-lg border px-3 py-1.5 text-sm font-medium hover:bg-[var(--shell-hover)] transition-colors" style={{ borderColor: "var(--shell-card-border)", color: "var(--shell-text)" }}>
                ↓ Excel
              </button>
        ...
        <div className="mt-5 overflow-hidden rounded-xl border" style={{ borderColor: "var(--shell-card-border)", background: "var(--shell-card-bg)" }}>
          ...
          {showFilters && (
            <div className="grid gap-2 border-b px-4 py-2 items-center"
              ...
              {/* Nome: livre (busca global já cobre) */}
              <div />
              {/* Telefone: vazio */}
              <div />
              {/* Origem */}
