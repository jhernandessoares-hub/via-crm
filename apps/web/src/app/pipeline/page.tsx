...
export default function PipelinePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((l) => {
      if (qq) {
        ...
        const match = [l.nome, l.nomeCorreto, l.telefone, l.whatsapp, l.origem, l.status, l.perfilImovel, sn, ind, num, l.assignedUserName]
  ...
  const pendingLeads = useMemo(() => filtered.filter((l) => l.conversaAberta === true), [filtered]);
  const normalLeads  = useMemo(() => filtered.filter((l) => l.conversaAberta !== true), [filtered]);
    ...
    <AppShell title="Todos os Leads">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-lg border p-1 gap-0.5"
