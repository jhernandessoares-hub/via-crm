...
function ObraUpdateCard({ devId, update, onDeleted, onUpdated }: {
  ...
  const [editData, setEditData] = useState({ dataAtualizacao: update.dataAtualizacao.slice(0, 10), titulo: update.titulo || "", observacoes: update.observacoes || "" });
...
function ObraEvolution({ devId }: { devId: string }) {
  ...
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ dataAtualizacao: new Date().toISOString().slice(0, 10), titulo: "", observacoes: "" });
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ dataAtualizacao: new Date().toISOString().slice(0, 10), titulo: "", observacoes: "" });
    } finally {
      setSaving(false);
    }
  }
