  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer select-none group">
      <button
        ...
        className={`relative h-5 w-9 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-slate-400 ${checked ? "bg-slate-800" : "bg-[var(--shell-card-border)]"}`}
      >
        <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-150 ${checked ? "translate-x-4" : "translate-x-0.5"}`} />
      </button>
      <span className="text-sm text-[var(--shell-subtext)] group-hover:text-[var(--shell-text)] transition-colors">{label}</span>
    </label>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="block text-xs font-semibold text-[var(--shell-subtext)] tracking-wide">{label}</label>
      {children}
    </div>
  );
}
    ...
    <div className="space-y-2">
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((tag) => (
            <span key={tag} className="inline-flex items-center gap-1 rounded-full border border-[var(--shell-card-border)] bg-[var(--shell-bg)] px-2.5 py-1 text-xs font-medium text-[var(--shell-text)]">
              {tag}
        ...
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="w-60 shrink-0 space-y-3 sticky top-4">
            ...
            {(product as any)?.capturedBy && (
              <div className="rounded-xl border border-[var(--shell-card-border)] bg-[var(--shell-card-bg)] px-4 py-4 shadow-sm">
