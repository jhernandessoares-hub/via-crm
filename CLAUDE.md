...
- `/gestao-empreendimentos/novo` → Criação com mapa Google Maps para lat/lng + orientação solar.
- `/gestao-empreendimentos/[id]` → Detalhe com 4 abas: Cadastro (dados + implantação + torres com dimensões 3D), Espelho (2D color-coded + 3D Three.js com walk-through FPS), Preços (tabela + condições de pagamento), Dashboard (VSO, VGV, gráfico). O espelho 2D diferencia VERTICAL (grade por andar) de HORIZONTAL/CASA e HORIZONTAL/LOTEAMENTO (top-down lotes). O 3D usa Three.js com WebGLOverlayView (Google Maps), geração procedural por tipo (prédios, casas com telhado, lotes), raycasting para seleção de unidades e PointerLockControls para passeio virtual FPS. Env necessária: `NEXT_PUBLIC_GOOGLE_MAPS_KEY` (Maps JS API). Deps: `three`, `@types/three`, `@googlemaps/js-api-loader`.
